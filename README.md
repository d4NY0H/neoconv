...
- **`--v-bank-size` option and smarter V-ROM auto detection** (`extract` command and GUI): V-ROM is currently always split into fixed 2 MB chunks (`V_BANK_SIZE`), which is correct for only 22.5% of the MAME library. A `--v-bank-size` flag is planned with the following modes:
  - `auto` — uses the same validated heuristic developed for C chips: compute `v_total / chip_count` and round up to the nearest standard chip size (512 KB, 1, 2, 4, 8, 16 MB). Tested against the full MAME Neo Geo software list (307 games), this achieves 99% accuracy.
  - Manual values: `512k`, `1m`, `2m`, `4m`, `8m`, `16m` — explicit chip size for games where auto gives the wrong result.
  - `mixed` — hardcoded split of `[1 MB, 512 KB, 512 KB]`, covering the three known games with mixed V chip sizes: **superspy**, **gpilots**, and **gpilotsh**. These are the only games in the entire Neo Geo library with this configuration and cannot be handled by a single bank size value.
- **Smarter `--c-chip-size auto` heuristic**: the current auto mode derives chip size as `c_total / 2`, which is only correct for games with exactly two C chips (21.5% of the MAME library). A validated heuristic has been designed: compute `c_total / chip_count` (from the `.neo` header) and round up to the nearest standard chip size (512 KB, 1, 2, 4, 8, 16, 20 MB). Tested against the full MAME Neo Geo software list (307 games), this approach achieves 99% accuracy. The three remaining edge cases are bootlegs or hacks with non-standard padding. Implementation is planned for a future release.
