...
- **Smarter `--c-chip-size auto` heuristic with 100% MAME coverage** (`extract` command and GUI): the current auto mode derives chip size as `c_total / 2`, which is only correct for games with exactly two C chips (21.5% of the MAME library). The planned implementation uses two mechanisms:
  - **Primary heuristic**: compute `c_total / chip_count` and round up to the nearest standard chip size (512 KB, 1, 2, 4, 8, 16, 20 MB). Validated against the full MAME Neo Geo software list (307 games), this achieves 99% accuracy.
  - **M1 SHA1 fingerprint lookup**: for the three games with mixed C chip sizes that the heuristic cannot resolve, the M1 ROM (audio CPU, always a single chip in MAME and stored verbatim in `.neo`) is SHA1-hashed at extract time and matched against a small hardcoded table. This yields exact chip layouts for **whp** (`090-m1.m1`, sha1 `0c60d4af...`: `[4, 4, 2, 2, 2, 2, 2, 2] MB`), **kizuna** (`216-m1.m1`, sha1 `72852e78...`: `[2, 2, 4, 4, 2, 2, 2, 2] MB`), and **kof97oro** (`orochi-m1.bin`, sha1 `ed77cbae...`: `[16, 16, 2, 2, 2, 2] MB`). Together these two mechanisms achieve **100% coverage** of the MAME Neo Geo library with no user intervention required.
- **`--v-bank-size` option and smarter V-ROM auto detection with 100% MAME coverage** (`extract` command and GUI): V-ROM is currently always split into fixed 2 MB chunks (`V_BANK_SIZE`), which is correct for only 22.5% of the MAME library. The planned implementation mirrors the C chip approach:
  - **Primary heuristic**: compute `v_total / chip_count` and round up to the nearest standard chip size (512 KB, 1, 2, 4, 8, 16 MB). Validated against the full MAME Neo Geo software list (307 games), this achieves 99% accuracy. Manual values (`512k`, `1m`, `2m`, `4m`, `8m`, `16m`) will also be available for edge cases.
  - **M1 SHA1 fingerprint lookup**: for the three games with mixed V chip sizes, the same M1 fingerprint mechanism resolves the correct layout. **superspy** (`011-m1.m1`, sha1 `4e3cb57d...`: `[1 MB, 512 KB, 512 KB]`) and **gpilots/gpilotsh** (shared M1 sha1 `0e212d2c...`: `[1 MB, 512 KB, 512 KB]`). This achieves **100% coverage** of the MAME Neo Geo library with fully automatic detection.
