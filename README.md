...
- **`--v-bank-size` option and smarter V-ROM auto detection** (`extract` command and GUI): V-ROM is currently always split into fixed 2 MB chunks (`V_BANK_SIZE`), which is correct for only 22.5% of the MAME library. A `--v-bank-size` flag analogous to `--c-chip-size` is planned, along with an auto mode using the same validated heuristic developed for C chips: compute `v_total / chip_count` and round up to the nearest standard chip size (512 KB, 1, 2, 4, 8, 16 MB). Tested against the full MAME Neo Geo software list (307 games), this achieves 99% accuracy. The three remaining edge cases (superspy, gpilots, gpilotsh) have mixed chip sizes (1 MB + 2× 512 KB) where the average rounds up incorrectly.
